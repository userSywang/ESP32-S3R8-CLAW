...
esp_err_t ec_embed_claw_start(void)
{
    ...
    if (s_started) {
        ESP_LOGI(TAG, "Claw already started, skip duplicate start");
        return ESP_OK;
    }
...
static void health_monitor_task(void *arg)
{
    ...
    while (1) {
        ...
        bool openclaw_connected = false;
        uint32_t feishu_connect_count = 0;
        uint32_t feishu_disconnect_count = 0;
        uint32_t openclaw_connect_count = 0;
        uint32_t openclaw_disconnect_count = 0;
        int64_t last_event_age_ms = -1;
        int64_t last_send_age_ms = -1;
        int64_t openclaw_last_recv_ms = -1;
        int64_t openclaw_last_send_ms = -1;
        ...
        ec_channel_feishu_get_health(&feishu_connected,
                                     ...
                                     &last_event_age_ms,
                                     &last_send_age_ms);
        ec_channel_openclaw_get_health(&openclaw_connected,
                                       &openclaw_connect_count,
                                       &openclaw_disconnect_count,
                                       &openclaw_last_recv_ms,
                                       &openclaw_last_send_ms);

        ESP_LOGI(TAG,
                 "health wifi=%s ip=" IPSTR " rssi=%d heap=%u psram=%u feishu_ws=%s connect=%" PRIu32 " disconnect=%" PRIu32 " last_event_ms=%" PRId64 " last_send_ms=%" PRId64 " openclaw=%s oc_connect=%" PRIu32 " oc_disconnect=%" PRIu32 " oc_last_recv_ms=%" PRId64 " oc_last_send_ms=%" PRId64 " outbound_stack=%u",
