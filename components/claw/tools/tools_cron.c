...
static ec_cron_job_t *cron_find_job_by_id(const char *job_id);
...
esp_err_t ec_tools_cron_mark_dispatch_result(const char *job_id, bool success)
{
    ec_cron_job_t *job = cron_find_job_by_id(job_id);
    ...
    if (job->kind != CRON_KIND_AT || !job->delete_after_run) {
        return ESP_OK;
    }

    if (success) {
        ESP_LOGI(TAG, "Cron one-shot delivered successfully: %s (%s)", job->name, job->id);
        return ec_cron_remove_job(job->id);
    }

    if (job->dispatch_attempts > job->max_retries) {
        ESP_LOGW(TAG, "Cron one-shot exhausted retries: %s (%s)", job->name, job->id);
        return ec_cron_remove_job(job->id);
    }

    job->enabled = true;
    job->next_run = now + EC_CRON_ONESHOT_RETRY_DELAY_S;
    ESP_LOGW(TAG, "Cron one-shot failed, retry scheduled in %ds: %s (%s) attempt=%u/%u",
             EC_CRON_ONESHOT_RETRY_DELAY_S, job->name, job->id,
             (unsigned)job->dispatch_attempts, (unsigned)(job->max_retries + 1));
...
static char *cron_build_dispatch_message(const ec_cron_job_t *job)
{
    ...
    len = snprintf(NULL, 0, prefix, job->id) + strlen(job->message) + 1;
    msg = (char *)malloc(len);
    if (!msg) {
        return NULL;
    }

    snprintf(msg, len, prefix, job->id);
    strncat(msg, job->message, len - strlen(msg) - 1);
    return msg;
}
...
static esp_err_t cron_save_jobs(void)
{
    ...
    for (int i = 0; i < s_job_count; i++) {
        ...
        cJSON_AddNumberToObject(item, "dispatch_attempts", job->dispatch_attempts);
        cJSON_AddNumberToObject(item, "max_retries", job->max_retries);
...
static esp_err_t cron_load_jobs(void)
{
    ...
    cJSON_ArrayForEach(item, jobs_arr) {
        ...
        cJSON *dispatch_attempts = cJSON_GetObjectItem(item, "dispatch_attempts");
        cJSON *max_retries = cJSON_GetObjectItem(item, "max_retries");
        ...
        job.dispatch_attempts = cJSON_IsNumber(dispatch_attempts) ? (uint8_t)dispatch_attempts->valuedouble : 0;
        job.max_retries = cJSON_IsNumber(max_retries) ? (uint8_t)max_retries->valuedouble : EC_CRON_ONESHOT_MAX_RETRIES;
    ...
    for (int i = 0; i < s_job_count; i++) {
        ...
        if (msg.content) {
            ...
            if (err != ESP_OK) {
                ...
                if (job->kind == CRON_KIND_AT && job->delete_after_run) {
                    ...
                    (void)ec_tools_cron_mark_dispatch_result(job->id, false);
                }
                changed = false;
                continue;
            }
        ...
        if (job->kind == CRON_KIND_AT) {
            if (job->delete_after_run) {
                ...
                job->enabled = true;
                job->next_run = now + EC_CRON_ONESHOT_RETRY_DELAY_S;
                ESP_LOGI(TAG, "One-shot job dispatched, awaiting delivery result: %s (%s) attempt=%u/%u",
                         job->name, job->id,
                         (unsigned)job->dispatch_attempts,
                         (unsigned)(job->max_retries + 1));
            } else {
                job->enabled = false;
                job->next_run = 0;
            }
...
static esp_err_t ec_tool_cron_add_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    if (strcmp(schedule_type, "every") == 0) {
        ...
        job.max_retries = job.delete_after_run ? EC_CRON_ONESHOT_MAX_RETRIES : 0;
    } else {
