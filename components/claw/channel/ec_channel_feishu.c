...
typedef enum {
    FEISHU_WS_STATE_DISCONNECTED = 0,
    FEISHU_WS_STATE_CONNECTING,
    FEISHU_WS_STATE_CONNECTED,
} feishu_ws_state_t;
...
static volatile feishu_ws_state_t s_ws_state = FEISHU_WS_STATE_DISCONNECTED;
...
static int64_t s_ws_accept_after_ms = 0;
...
static int64_t current_wall_time_ms(void)
{
...
static void process_ws_event_payload(const char *payload, size_t payload_len)
{
    ...
    cJSON *message_create_time_node = cJSON_GetObjectItem(message, "create_time");
    char open_id_buf[48] = {0};
    char chat_id_buf[48] = {0};
    char chat_type_buf[16] = "p2p";
    int64_t message_create_ms = 0;
    ...
    if (message_create_time_node) {
        if (cJSON_IsString(message_create_time_node) && message_create_time_node->valuestring) {
            message_create_ms = atoll(message_create_time_node->valuestring);
        } else if (cJSON_IsNumber(message_create_time_node)) {
            message_create_ms = (int64_t)message_create_time_node->valuedouble;
        }
    }
    if (s_ws_accept_after_ms > 0 && message_create_ms > 0 &&
        message_create_ms < s_ws_accept_after_ms) {
        ESP_LOGI(TAG, "Feishu WS: stale message filtered (create_time=%lld, accept_after=%lld)",
                 (long long)message_create_ms, (long long)s_ws_accept_after_ms);
        goto ws_data_done;
    }
...
static void handle_ws_event(void *arg, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case WEBSOCKET_EVENT_CONNECTED: {
        int64_t now_us = esp_timer_get_time();
        int64_t now_ms = current_wall_time_ms();
        ESP_LOGI(TAG, "Feishu WS connected");
        s_ws_connected = true;
        s_ws_state = FEISHU_WS_STATE_CONNECTED;
        s_ws_connect_count++;
        s_last_ping_us = now_us;
        s_ws_connected_at_us = now_us;
        s_ws_accept_after_ms = now_ms > 0 ? now_ms - EC_FEISHU_STALE_EVENT_MAX_AGE_MS : 0;
        break;
    }
    case WEBSOCKET_EVENT_DISCONNECTED:
        ESP_LOGW(TAG, "Feishu WS disconnected");
        s_ws_connected = false;
        s_ws_state = FEISHU_WS_STATE_DISCONNECTED;
...
static void feishu_ws_task(void *arg)
{
    ...
    while (1) {
        ...
        s_ws_url[0] = '\0';
        s_service_id[0] = '\0';
        s_ping_interval_s = EC_FEISHU_PING_INTERVAL_S;
        ...
        s_ws_connected = false;
        s_ws_state = FEISHU_WS_STATE_CONNECTING;
        esp_websocket_register_events(s_ws_client, WEBSOCKET_EVENT_ANY, handle_ws_event, NULL);
        esp_websocket_client_start(s_ws_client);
        int wait_connect_ms = 0;
        while (s_ws_state == FEISHU_WS_STATE_CONNECTING && wait_connect_ms < 20000) {
            vTaskDelay(pdMS_TO_TICKS(200));
            wait_connect_ms += 200;
        }
        if (s_ws_state != FEISHU_WS_STATE_CONNECTED) {
            ESP_LOGW(TAG, "Feishu WS connect timeout or failed, will refresh endpoint and reconnect");
        ...
        while (s_ws_state == FEISHU_WS_STATE_CONNECTED && s_ws_client &&
        ...
        ESP_LOGW(TAG, "Feishu WS loop ended, will refresh endpoint and reconnect");
        esp_websocket_client_stop(s_ws_client);
        esp_websocket_client_destroy(s_ws_client);
        s_ws_client = NULL;
        s_ws_connected = false;
...
static esp_err_t ec_channel_feishu_send(const ec_msg_t *msg)
{
    ...
    while (attempt < 2) {
        ...
        tlen = strlen(text);
        chunk = (tlen > FEISHU_MAX_MSG_LEN) ? FEISHU_MAX_MSG_LEN : tlen;
        escaped = malloc(chunk * 2 + 32);
        if (!escaped) {
            return ESP_ERR_NO_MEM;
        }
        size_t j = 0;
        ...
        content_json = cJSON_PrintUnformatted(content_inner);
        ...
        ESP_LOGI(TAG, "Sending message to Feishu (attempt=%d, chat_id=%s, text=%.*s)",
                 attempt + 1, chat_id, (int)chunk, text);
        cJSON_Delete(body);
        free(content_json);
        if (!body_str) {
            return ESP_ERR_NO_MEM;
        ...
        if (!(err == ESP_ERR_HTTP_CONNECT || err == ESP_ERR_HTTP_FETCH_HEADER || err == ESP_ERR_HTTP_WRITE_DATA || err == ESP_ERR_TIMEOUT || err == ESP_FAIL)) {
            return ret;
        }

        attempt++;
        if (attempt < 2) {
            ESP_LOGW(TAG, "Feishu send transient failure (%s), retrying once", esp_err_to_name(err));
